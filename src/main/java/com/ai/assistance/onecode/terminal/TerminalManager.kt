...
import com.ai.assistance.onecode.terminal.data.TerminalState
import com.ai.assistance.onecode.terminal.data.CommandHistoryItem
import com.ai.assistance.onecode.terminal.data.QueuedCommand
import com.ai.assistance.onecode.terminal.view.domain.OutputProcessor
...
import com.ai.assistance.onecode.terminal.utils.SSHDServerManager
import com.ai.assistance.onecode.terminal.provider.filesystem.FileSystemProvider
import com.ai.assistance.onecode.terminal.provider.filesystem.LocalFileSystemProvider
import com.ai.assistance.onecode.terminal.provider.type.TerminalProvider
import com.ai.assistance.onecode.terminal.provider.type.TerminalType
import com.ai.assistance.onecode.terminal.provider.type.LocalTerminalProvider
import com.ai.assistance.onecode.terminal.provider.type.SSHTerminalProvider
...
class TerminalManager private constructor(
    ...
    suspend fun createNewSession(
        ...
        val success = withTimeoutOrNull(30000) { // 30秒超时
            terminalState.first { state ->
                ...
                session?.initState == com.ai.assistance.onecode.terminal.data.SessionInitState.READY
            }
        }
    ...
    private fun generateStartScript(): String {
        ...
        val installUbuntu = """
        install_ubuntu(){
          OK_FILE="${'$'}UBUNTU_PATH/.onecode_installed_ok"
          ...
          if [ -f "${'$'}OK_FILE" ]; then
            ...
            if [ -f "${'$'}UBUNTU_PATH/etc/issue.net" ]; then
              ...
              echo "ok" > "${'$'}TMP_DIR/.onecode_installed_ok" 2>/dev/null || true
        ...
        # From Onecode Settings - ${aptSource.name}
    ...
    private fun handleRegularCommand(command: String, session: com.ai.assistance.onecode.terminal.data.TerminalSessionData, commandId: String) {
